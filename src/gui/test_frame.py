...
class TestFrame(ttk.Frame):
    ...
    def add_input_components(self, x, y, width, height):
        ...
        correct_btn = ttk.Button(
            self.input_frame, text="Correct Glyph", command=self.gui_correct
        )
        ...
        g2p_btn = ttk.Button(
            self.input_frame,
            text="Syllable & Phoneme",
            command=self.gui_g2p,
            state=tk.DISABLED,
        )
    ...
    def add_output_components(self, x, y, width, height):
        ...
        self._mm_ti = TestItem(
            self._output_frame, self._output_titles[1], x, y=y_var_out(2)
        )
        self._output_functions.append(self._mm_ti)

        self._syl_ti = TestItem(
            self._output_frame, self._output_titles[2], x, y=y_var_out(3)
        )
        self._output_functions.append(self._syl_ti)
    ...
    def gui_transliterate(self) -> None:
        ...
        text = self.text
        self.reset(True)
        self._mm_ti.set_values(self.transliterate(text))
    ...
    def gui_correct_and_transliterate(self) -> None:
        ...
        self._mm_ti.set_values(self.transliterate(str_bn))

    def gui_syl(self) -> None:
        """
        Perform the Syllable division for Meetei Mayek in the GUI.
